...
{
    function index(){
        $data['data'] = Pengembalian::select("pengembalians.*","peminjaman.kode","daftar_books.cover_buku","daftar_books.judul_buku","daftar_books.pengarang","daftar_books.tahun_terbit")
                                ->leftJoin('peminjaman','peminjaman.id','=','pengembalians.id_peminjaman')
                                ->leftJoin('daftar_books','daftar_books.id','=','peminjaman.id_buku')
                                ->get();
        $data['datacount'] = Pengembalian::select("pengembalians.*","peminjaman.kode","daftar_books.cover_buku","daftar_books.judul_buku","daftar_books.pengarang","daftar_books.tahun_terbit")
                                ->leftJoin('peminjaman','peminjaman.id','=','pengembalians.id_peminjaman')
                                ->leftJoin('daftar_books','daftar_books.id','=','peminjaman.id_buku')
                                ->count();
    ...
    public function update(Request $request)
    {
        ...
        $data->ulasan = Request()->ulasan;
        $data->save();
        return redirect()->route('pengembalian')->with('success', 'Pengembalian Buku Berhasil Dikembalikan!');
    }
    function delete($id){
